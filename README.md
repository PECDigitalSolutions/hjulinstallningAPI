...
🚗 Vehicle API
Method	Endpoint	Description
GET	/api/vehicle/{licensePlate}	Fetches vehicle data from cache, DB, or D&B API
POST	/api/vehicle/set	Adds test vehicle data (stored locally)
⚔️ WowCharacter API
Method	Endpoint	Description
GET	/api/wowcharacter	Get all WoW characters
POST	/api/wowcharacter	Add a new character
PUT	/api/wowcharacter/{id}	Update a character
DELETE	/api/wowcharacter/{id}	Remove a character
📌 Testing in Swagger
